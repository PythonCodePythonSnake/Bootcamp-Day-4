...
missing. Does NOT perform travel research — that's the planner/specialized
agents' job.
"""
...
def _detect_missing(travel_request: TravelRequest) -> MissingInfo:
    ...
    has_exact_dates = bool(travel_request.dates.start_date and travel_request.dates.end_date)
    if not has_duration and not has_exact_dates:
        missing_fields.append("dates")
        questions.append("What dates, or how many days, is your trip?")

    if travel_request.budget.amount is None:
        missing_fields.append("budget")
        questions.append("What is your approximate budget for this trip?")

    return MissingInfo(fields=missing_fields, questions=questions)


def run(state: TravelState) -> TravelState:
    """Extract/update the structured TravelRequest from the latest user input.

    Returns a partial state update (LangGraph merges this into the full state).
    """
    ...
    if existing:
        context = (
            "\n\nPreviously extracted information — merge with and update this, "
            f"do not discard known fields unless contradicted:\n{existing.model_dump_json()}"
        )
    ...
    except Exception as exc:  # LLM/config failure — degrade gracefully, ask the human
        errors = state.get("errors", [])
        errors.append({"node": "orchestrator", "message": str(exc), "retry_count": 0})
        fallback_request = existing or TravelRequest(raw_text=user_input)
